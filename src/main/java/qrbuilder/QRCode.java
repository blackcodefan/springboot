...
import qrbuilder.decorator.Decorator;
import qrbuilder.exception.CouldNotCreateFileException;
import qrbuilder.exception.CouldNotCreateQRCodeException;
import qrbuilder.exception.InvalidSizeException;
import qrbuilder.exception.UnreadableDataException;
import qrbuilder.util.SyntacticSugar;
...
public class QRCode {
    ...
    public File toFile(File imageFile, String fileFormat) throws CouldNotCreateQRCodeException, UnreadableDataException {
        SyntacticSugar.throwIllegalArgumentExceptionIfEmpty(fileFormat, "fileFormat");
...
//--------------------
    ...
    private Result decode(BufferedImage qrcode) throws FormatException, ChecksumException, NotFoundException {
        return new QRCodeReader().decode(new BinaryBitmap(new HybridBinarizer(new BufferedImageLuminanceSource(qrcode))), getDecodeHints()
        );
    }
